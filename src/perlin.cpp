...
{
    // Resize permutation vector to 256 and populate with values ranging from
    // 0 to 255.
    p.resize(256);
    std::iota(p.begin(), p.end(), 0);

    // Initialise a PRNG with the seed. Shuffle using PRNG and duplicate the
    // permutation vector.
    std::default_random_engine rengine(seed);
    std::shuffle(p.begin(), p.end(), rengine);
